<?
	...
	if ($Pass == "Kirk4226652") {
		...
		if ($Long == "" or $Long == "false") {
			$ch = curl_init("https://messages.att.net/messages/v0/threads?includeContactInfo=true&limit=10&offset=0&threadBy=contact&depth=$Depth");
			curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
			...
			$savin = md5($result);
			mysqli_query($con,"UPDATE Cache SET Source='$savin' WHERE ID='1'");
		} else {
			$con = $con = mysqli_connect('storage.tn-studio.com', 'rbxdatabase', 'ys5f4uifkr5rlpwgacxy5sbnjzhykzym2r1uzj06r0mwptt4r7c3b5zfmo5d3sff', 'ScriptCache');
			$cache = mysqli_fetch_assoc(mysqli_query($con, "SELECT Source FROM Cache WHERE ID='1'"))["Cache"];
			$time = time();
			while (true) {
				$ch = curl_init("https://messages.att.net/messages/v0/threads?includeContactInfo=true&limit=10&offset=0&threadBy=contact&depth=$Depth");
				curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
